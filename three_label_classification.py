...
'''
def one_hot_encoding(seq, k, kmer_encoding):
...
def fasta_to_vectors(in_fasta, k):
    ...
    seq_vector = [one_hot_encoding(x, k, kmer_encoding) for x in seq]
    return seq_vector
'''
...
def main():
    ...
    lab1_fasta = sys.argv[1]
    lab2_fasta = sys.argv[2]
    lab3_fasta = sys.argv[3]
    weight_file = sys.argv[4]
    kmers = sys.argv[5].split(',')
    k = np.array([int(i) for i in kmers])
    l = int(sys.argv[6])
